...
class Zigbee2Mqtt(MqttPlugin):
    ...
    # ToDo:
        # Add further sensors and switches
    ...
    def __init__(self, sh):
        ...
        self.zigbee2mqtt_devices = {}            # to hold device information for web interface
        self.zigbee2mqtt_plugin_devices = {}     # to hold device information for web interface
        self.zigbee2mqtt_items = []              # to hold item information for web interface
        self.zigbee2mqtt_meta = {}               # to hold meta information for web interface
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'zigbee2mqtt_topic'):
            ...
            if zigbee2mqtt_attr != '':
                ...
                if not item in self.zigbee2mqtt_items:
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        # code to execute if the plugin is not stopped  AND only, if the item has not been changed for this plugin
            ...
            # get zigbee2mqtt attributes of item
            topic_level2 = self.get_iattr_value(item.conf, 'zigbee2mqtt_topic')
            zigbee2mqtt_attr = self.get_iattr_value(item.conf, 'zigbee2mqtt_attr')
            
            if zigbee2mqtt_attr in ['bridge_permit_join', 'bridge_health_check', 'bridge_restart', 'bridge_networkmap_raw', 'device_remove', 'device_ota_update_check', 'device_ota_update_update', 'device_configure', 'device_options', 'device_rename', 'device_bind', 'device_unbind', 'device_configure_reporting']:
    ...
    def on_mqtt_announce(self, topic, payload, qos=None, retain=None):
        ...
        if len(wrk) > 2:
            ...
            if len(wrk) > 3:
                ...
                if len(wrk) > 4:
                    topic_level5 = wrk[4]
        self.logger.info(f"on_mqtt_announce: topic_level1={topic_level1}, topic_level2={topic_level2}, topic_level3={topic_level3}, topic_level4={topic_level4}, topic_level5={topic_level5}, payload={payload}")

        # type(var) is dict oder isinstance(var, dict)
        # wanted_key in var oder     var.get(wanted_key)
        # type(var[wanted_key]) is dict / list / ...     oder   isinstance(var[wanted_key], dict)
        # wanted_key2 in var[wanted_key]
        # if var[wanted_key][wanted_key2]: ..
                    ...
                    if topic_level3 == 'response':
                        ...
                        if topic_level4 is '':
                            ...
                            if 'message' in payload:
                                if type(payload['message']) is list:
                                    for element in payload['message']:
                                        device = element.get('friendly_name')
                                        if device:
                                            if 'lastSeen' in element:
                                                element.update({'lastSeen': datetime.fromtimestamp(element['lastSeen']/1000)})
                                    ...
                                    self.logger.debug(f"bridge log message was {payload} but not processed.")
                        ...
                        if topic_level4 is '':
                            if type(payload) is dict:
                                ...
                                self.zigbee2mqtt_plugin_devices[topic_level2]['online'] = True
                            self.logger.debug(f"bridge info message not implementerd yet.")
                        ...
                        if topic_level4 is '':
                            ...
                            self.logger.debug(f"event info message not implementerd yet.")
            
                else:
                    self.logger.debug(f"received payload type is not of type dict")


        elif topic_level2 in list(self.zigbee2mqtt_devices.keys()):
            if topic_level3 + topic_level4 + topic_level5 == '':
                # topic_level1=zigbee2mqtt, device=SNZB02_01, topic_level3=None, topic_level4=None, topic_level5=None, payload '{"battery":100,"device":{"applicationVersion":5,"dateCode":"20201026","friendlyName":"SNZB02_01","hardwareVersion":1,"ieeeAddr":"0x00124b00231e45b8","manufacturerID":0,"manufacturerName":"eWeLink","model":"SNZB-02","networkAddress":18841,"powerSource":"Battery","type":"EndDevice","zclVersion":1},"humidity":45.12,"linkquality":157,"temperature":16.26,"voltage":3200}'
                # topic_level1=zigbee2mqtt, device=TRADFRI E1766_01, topic_level3=None, topic_level4=None, topic_level5=None, payload={'battery': 74, 'device': {'applicationVersion': 33, 'dateCode': '20190311', 'friendlyName': 'TRADFRI E1766_01', 'hardwareVersion': 1, 'ieeeAddr': '0x588e81fffe28dec5', 'manufacturerID': 4476, 'manufacturerName': 'IKEA of Sweden', 'model': 'E1766', 'networkAddress': 39405, 'powerSource': 'Battery', 'softwareBuildID': '2.2.010', 'stackVersion': 98, 'type': 'EndDevice', 'zclVersion': 3}, 'linkquality': 141}
                
                if type(payload) is dict:
                    if 'device' in payload:
                        meta = payload['device']
                        # Umbenennen des Key 'friendlyName', damit er identisch zu denen aus den Log und Config Topcis ist
                        ...
                        if topic_level2 in list(self.zigbee2mqtt_plugin_devices.keys()):
                            self.logger.debug(f"Item to be checked for update and updated")
                            if type(payload) is dict:
                                for element in payload:
